...
export default function AgentBuilderPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-4">{agent.display_name || agent.name}</h1>
        ...
        <div className="flex gap-4">
          {tabs.map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`pb-2 text-sm font-medium border-b-2 ${tab === t ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              {t}
            </button>
          ))}
        </div>
      ...
      {tab === 'Config' && (
        <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Model Deployment</label>
            <input value={config.model_deployment} onChange={(e) => setConfig({ ...config, model_deployment: e.target.value })}
              className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Prompt Stack (JSON)</label>
            <textarea value={config.prompts} onChange={(e) => setConfig({ ...config, prompts: e.target.value })}
              className="w-full border border-gray-300 rounded px-3 py-2 text-sm font-mono" rows={6} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tools Config (JSON)</label>
      ...
      {tab === 'Chat' && (
        <div className="bg-white rounded-lg border border-gray-200 flex flex-col" style={{ height: 'calc(100vh - 280px)' }}>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && <p className="text-gray-400 text-sm">Send a message to start chatting with the agent.</p>}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] rounded-lg px-3 py-2 text-sm ${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                  <p className="whitespace-pre-wrap">{m.content}</p>
                  {m.tool_calls?.map((tc, j) => (
                    <div key={j} className="mt-2 text-xs bg-gray-200 rounded p-2">
                      <p className="font-semibold">Tool: {tc.name}</p>
                      <pre className="mt-1 overflow-x-auto">{JSON.stringify(tc.input, null, 2)}</pre>
                      {tc.output && <pre className="mt-1 text-gray-600">{tc.output}</pre>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="border-t border-gray-200 p-3 flex gap-2">
            <input value={input} onChange={(e) => setInput(e.target.value)}
              ...
              placeholder="Type a message..." className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm" />
      ...
      {tab === 'YAML' && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <pre className="text-sm font-mono text-gray-800 whitespace-pre-wrap overflow-x-auto">{yaml || 'Loading...'}</pre>
        </div>
      )}
    </div>
  )
